...
const processTopics = (topics) => (topics || []).map((topic) => topic.replace(/([ _])/g, "-").toLowerCase());
...
const createRepoDataWithResponse = (json, monorepo) => {
  ...
  if (!monorepo) {
    json.lastRelease =
      json.releases && json.releases.nodes && json.releases.nodes.length ? json.releases.nodes[0] : undefined;
  }
