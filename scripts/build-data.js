...
import githubRepos from "../cocoa-libraries.json";
...
export const sleep = (ms = 0, msMax = null) => {
  return new Promise((r) => setTimeout(r, msMax ? Math.floor(Math.random() * (msMax - ms)) + ms : ms));
};
...
const buildAndScoreData = async () => {
  ...
  // const CHUNK_SIZE = 32;
  // bulkList = [...Array(Math.ceil(bulkList.length / CHUNK_SIZE))].map((_) => bulkList.splice(0, CHUNK_SIZE));

  // const downloadsList = (await Promise.all(bulkList.map((chunk) => fetchNpmDataBulk(chunk)))).flat();
  // const downloadsListWeek = (await Promise.all(bulkList.map((chunk) => fetchNpmDataBulk(chunk, "week")))).flat();
